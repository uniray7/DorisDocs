...
- ⚠️ **與 RFC-003 全代建前提的衝突（2026-07-17）**：使用者帳號不授予 DDL 權限（database＋table 皆收回，DDL 一律平台代建）——但「帳號自理」若含 GRANT 權，使用者可自行開回 DDL 權限，技術強制破功。解法傾向**初始帳號不含 GRANT_PRIV、帳號/權限管理收回平台**（RFC-003 §8 隱患 3），可行性列 RFC-003 §11 首要待驗證；定案後本節改寫。
- 平台保留一個系統帳號做監控、保護性 config 維護與 **DDL 代建執行**（對外文件揭露其存在與用途）。
...
- Self-managed 承接的特有難題：這會成為「cluster 內權限使用者自理」的**唯一例外**。**政策漂移**（改表/刪表/重建後 policy 失效或漏掛）的風險因 2026-07-17 全代建改訂而降低——DDL 一律經平台之手，代建驗證時可攔截「此表掛有 row/column policy」並提醒重新宣告；但政策內容的更新責任仍在使用者。
