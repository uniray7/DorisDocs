...
平台 Kafka（per-ws topic + ACL）
   ...
   ├─ 驗證失敗 → S3/MinIO dead-letter（per-ws prefix）+ 告警
   ▼  驗證通過
...
3. 作為**資料負責人**，我想看到 pipeline 的 lag、吞吐與 dead-letter 數量並在異常時收到告警，以便及早處理格式錯誤或上游中斷。
4. 作為**平台維運**，我想要 format 錯誤的訊息自動進 dead-letter 而不阻塞 pipeline，以便單筆髒資料不影響整體匯入。
...
### 監控頁
- Pipeline 狀態（RUNNING / PAUSED / ERROR）、consumer lag、吞吐（rows/s、MB/s）、dead-letter 計數與樣本預覽、merge job 最近執行結果。
...
| 2 | Message format 驗證失敗 | 整筆落 dead-letter（含原始 payload + 失敗原因 + offset），計數告警（門檻可設）；pipeline 不中斷 |
...
| 8 | 配額 | tmp + dead-letter 占用計入 ws 儲存配額（防止殭屍 pipeline 無限累積） |
...
| GET | `/api/v1/workspaces/{ws}/pipelines/{id}/metrics` | lag/吞吐/dead-letter 指標 |
| GET | `/api/v1/workspaces/{ws}/pipelines/{id}/dead-letters` | 瀏覽 dead-letter 樣本 |
...
- Topic/憑證/dead-letter 的 per-ws 隔離依 **workspace-isolation**。
...
- [ ] 三種 format 各自可建 pipeline、落 tmp、dead-letter 行為一致
- [ ] format 驗證失敗不中斷 pipeline，dead-letter 含原始 payload 與失敗原因
- [ ] merge SQL 範圍檢核有效（無法寫入 tmp/curated 以外 ws 或未審核表）
- [ ] at-least-once 語義與冪等要求寫入對外文件，附 Unique Key 去重範例
- [ ] lag/dead-letter 告警可設門檻並送達 ws 聯絡人
- [ ] tmp/dead-letter 占用正確計入儲存配額
