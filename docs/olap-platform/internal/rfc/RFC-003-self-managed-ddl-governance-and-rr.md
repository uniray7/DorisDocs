...
2. **CCR 不是備份**。HA 方案是 cluster 級全同步——使用者誤 `DROP TABLE`、灌錯資料，standby 幾秒內同樣消失/污染。HA 擋機器故障，擋不住邏輯錯誤；「誤操作救援」的缺口只能靠備份補，而備份需要知道「備什麼」。
...
- 建立 database/table 時宣告（皆可日後修改）：
  - **備份**：要不要、頻率、保留份數 → 平台按宣告執行 Doris BACKUP 至 MinIO 並監控成功率
  - **TTL/retention**：要不要平台代清、保留多久 → 平台監控「宣告的清理有在生效」
...
| 備份執行與成功率監控 | ✅ **限有宣告的物件** | 宣告內容正確性；未宣告物件丟失自負 |
| 還原執行 | ✅（使用者申請後） | 提出申請、驗證還原結果 |
| TTL 清理生效監控 | ✅ 限有宣告的物件 | 宣告保留期的正確性 |
| 資料流向/血緣、資料正確性 | ❌ | ✅（寫入路徑在使用者手上） |
| 誤操作救援 | 有備份宣告→還原到最近快照；無宣告→❌（CCR 會同步誤操作，救不了） | 宣告涵蓋自己重要的表 |
| Row filtering / column masking | 待 open issue #3 決議（若承接，走同一套宣告機制） | 政策內容、schema 變更後重新宣告 |

> 選 Option 1/2 的差異：上表全部成立，另加「zone 段＝預設值包的宣告縮寫」（tmp 預設短 TTL 不備份、curated 預設要備份）——但預設值包在 Option 3 用表單預設值同樣做得到。**R&R 實質差異：無。**（同仁「option 2/3 R&R 無差別」的判斷正確，且在 table 代建為前提下，三個選項的 R&R 都幾乎相同。）
...
- [ ] Doris BACKUP/RESTORE 在 4.1.0 的物件粒度與與 CCR 並存行為
- [ ] 宣告清冊的存放與稽核（平台側系統）
